  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels >=
        ...
        ref.read(medicineNotifierProvider.notifier).fetchAllMedicines(loadMore: true);
      }
    });

    _searchController.addListener(() {
      ref.read(medicineNotifierProvider.notifier).search(_searchController.text);
    });
  }
                ...
                children: [
                  ...
                  Consumer(
                    builder: (context, ref, _) {
                      final count = ref.watch(cartNotifierProvider).items.length;
                      return count > 0
                          ? Positioned(
                        right: -5,
                        top: -5,
                      ...
                      child: PopupMenuButton<MedicineFilter>(
                        ...
                        itemBuilder: (context) => const [
                          PopupMenuItem(
                            ...
                            child: Text('None', style: TextStyle(fontWeight: FontWeight.normal, color: Colors.black, fontSize: 14)),
                          ),
                          PopupMenuItem(
                            value: MedicineFilter.lowToHigh,
                            child: Text('Price: Low to High', style: TextStyle(fontWeight: FontWeight.normal, color: Colors.black, fontSize: 14)),
                          ),
                          PopupMenuItem(
                            value: MedicineFilter.highToLow,
                            child: Text('Price: High to Low', style: TextStyle(fontWeight: FontWeight.normal, color: Colors.black, fontSize: 14)),
                          ),
                        ],
                      ),
                    ),
